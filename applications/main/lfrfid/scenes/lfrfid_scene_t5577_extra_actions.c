...
static void lfrfid_scene_t5577_extra_actions_submenu_callback(void* context, uint32_t index) {
    LfRfid* app = context;

    view_dispatcher_send_custom_event(app->view_dispatcher, index);
}

void lfrfid_scene_t5577_extra_actions_on_enter(void* context) {
    LfRfid* app = context;
    ...
    submenu_add_item(
        ...
        SubmenuIndexClearT5577,
        lfrfid_scene_t5577_extra_actions_submenu_callback,
        app);
    submenu_add_item(
        submenu,
...
bool lfrfid_scene_t5577_extra_actions_on_event(void* context, SceneManagerEvent event) {
    LfRfid* app = context;
    SceneManager* scene_manager = app->scene_manager;
    bool consumed = false;

    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == SubmenuIndexClearT5577) {
        ...
        dialog_message_set_buttons(message, "No", NULL, "Yes");
        //dialog_message_set_text(message, " This overwrites T5577 user data blocks.",  // Verify
...
void lfrfid_scene_t5577_extra_actions_on_exit(void* context) {
    LfRfid* app = context;
