...
static void lfrfid_wipe_t5577_do(LfRfid* app) {
	...
	if (!block) {	// page 0 block 0 is configuration data, default 0x00148040
	    data = (LFRFID_T5577_BITRATE_RF_64 |
		LFRFID_T5577_MODULATION_MANCHESTER | 
		(2 << LFRFID_T5577_MAXBLOCK_SHIFT) );
	} else
	    data = LFRFID_DATA_NULL;

	snprintf(curr_buf, sizeof(curr_buf), "Writing: Page %u Block %u\ndata %02lX %02lX %02lX %02lX", page, block, data>>24, (data>>16)&0xFF, (data>>8)&0xFF, data&0xFF);
	view_dispatcher_switch_to_view(app->view_dispatcher, LfRfidViewPopup);
	t5577_write_page_block_simple_with_start_and_stop(page, block, lock_bit, data/*, testmode*/);
	//furi_delay_ms(8);
	furi_delay_ms(150);
	// DEBUG delay
	if (!block)
