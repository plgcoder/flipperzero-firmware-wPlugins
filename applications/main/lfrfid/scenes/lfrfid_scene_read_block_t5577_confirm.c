...
void lfrfid_scene_read_block_t5577_confirm_on_enter(void* context) {
    LfRfid* app = context;
    //SceneManager* scene_manager = app->scene_manager;

    DialogMessage* message = dialog_message_alloc();
    dialog_message_set_header(message, "T5577 read data", 0, 0, AlignLeft, AlignTop);
    dialog_message_set_buttons(message, "0", NULL, "1");
    dialog_message_set_text(message, "Select page...", SCREEN_WIDTH_CENTER,
    ...
    uint8_t max_page_size = (app->read_page) ? LFRFID_PAGE1_MAX_SIZE : LFRFID_PAGE0_MAX_SIZE;
    dialog_message_set_header(message, "T5577 read data", 0, 0, AlignLeft, AlignTop);
    uint8_t left_box = app->read_block % max_page_size;
    //char str1[4];
    char* str1 = malloc(4);
...
bool lfrfid_scene_read_block_t5577_confirm_on_event(void* context, SceneManagerEvent event) {
    LfRfid* app = context;
    SceneManager* scene_manager = app->scene_manager;
    bool consumed = false;
    ...
    if(event.type == SceneManagerEventTypeBack) {
	    ...
	    scene_manager_next_scene(scene_manager, LfRfidSceneReadBlockT5577);
    }

    return consumed;
}

void lfrfid_scene_read_block_t5577_confirm_on_exit(void* context) {
    LfRfid* app = context;
